...
def alunos():
    ...
    l_email.place(x=4, y=71)
    e_l_email = Entry(frame_detalhes, width=45, justify='left', relief='solid')
    e_l_email.place(x=7, y=100)
    ...
    e_tel.place(x=7, y=150)

    # Sexo Masculino ou Feminino
    l_sexo = Label(frame_detalhes, text='Sexo *', height=1, anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
    l_sexo.place(x=188, y=140)
    c_sexo = ttk.Combobox(frame_detalhes, width=12, font=('Ivey 8 bold'))
    ...
    l_data_nascimento = Label(frame_detalhes, text="Data Nascimento", height=1, anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
    l_data_nascimento.place(x=446, y=10)
    data_inicio = DateEntry(frame_detalhes, width=14, background='darkblue', foreground='white', borderwidth=2, year=2025)
    data_inicio.place(x=450, y=40)

    #CPF
    l_cpf = Label(frame_detalhes, text='CPF *', height=1, anchor=NW, font=('Ivy 10'), bg=co1, fg=co4)
    l_cpf.place(x=446, y=80)
    ...
    c_turma = ttk.Combobox(frame_detalhes, width=12, font=('Ivey 8 bold'))
    c_turma['values'] = (turmas)
    c_turma.place(x=450, y=150)
    ...
    def escolher_imagem():
         ...
         botao_carregar['text'] = 'Tricar de Foto'
    ...
    # Linha separatoria lado esquedo cadastro 

    l_linha = Label(frame_detalhes, relief=GROOVE, text='h', width=1, height=100, anchor=NW, font=('Ivy 1'), bg=co0, fg=co0)
    l_linha.place(x=610, y=13)
    l_linha = Label(frame_detalhes, relief=GROOVE, text='h', width=1, height=100, anchor=NW, font=('Ivy 1'), bg=co1, fg=co0)
    l_linha.place(x=608, y=13)
          
    # Procutar aluno -----  

    l_nome = Label(frame_detalhes, text="Procurar Aluno [ Entrar o nome ]", height=1, anchor=NW, font=('Ivy 10'),bg=co1, fg=co4)
    l_nome.place(x=637, y=10)
    e_nome_procurar = Entry(frame_detalhes, width=17, justify='center', relief="solid", font=('Ivy 10'))
    e_nome_procurar.place(x=630, y=35)

    botao_procrurar = Button(frame_detalhes, anchor=CENTER, text="Procurar", width=9, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co1, fg=co0)
    botao_procrurar.place(x=757, y=35)

    #Botões alunos ----------

    botao_salvar = Button(frame_detalhes, anchor=CENTER, text="Salvar".upper(), width=9, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co6, fg=co1)
    botao_salvar.place(x=627, y=110)

    botao_atualizar = Button(frame_detalhes, anchor=CENTER, text="Atulizar".upper(), width=9, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co7, fg=co1)
    botao_atualizar.place(x=627, y=135)

    botao_deletar = Button(frame_detalhes, anchor=CENTER, text="Deletar".upper(), width=9, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co3, fg=co1)
    botao_deletar.place(x=627, y=160)

    botao_ver = Button(frame_detalhes, anchor=CENTER, text="Ver".upper(), width=9, overrelief=RIDGE, font=('Ivy 7 bold'), bg=co1, fg=co4)
    botao_ver.place(x=727, y=170)

    # Tabela Cadastro mostrar Alunos
    ...
    def mostrar_alunos():
        ...
        hd=["nw","nw","center", "center", "center", "center", "center", "center", "center",]
...
# Criando frames para tabelas ------
    frame_tabela_curso = Frame(frame_tabela, width=380, height=200, bg=co1)
    ...
    frame_tabela_turma = Frame(frame_tabela, width=380, height=200, bg=co1)
...
# botão cadastro
app_img_cadastro = Image.open('plus_sing.png')
...
app_img_adicionar = Image.open("plus_sing.png")
...
app_img_salvar = Image.open("disquete.png")
